...
from src.Player import Player, MatchRes
from src.Utility import CodeB, CodeSB
...
class Model(commands.Cog):  
    ...
    PRIV_CHAN_ID = 712861547617320982 #710245665896398899
    PRIV_CAT_ID = 712861096389771324 #711399900008415232
    ...
    def __enter__(self):
        self.bot = commands.Bot(command_prefix='!', help_command=commands.DefaultHelpCommand(command_attrs=dict(name='help'), width=120, dm_help=True)) #, status = '!help'))#, aliases=['hex_help', 'help_hex', 'hex-help'])))
    ...
    def ChkIsRegId(self, id: int):
        return self.playerDB.IsRegistered(id, Region.ALL)

    def RegFromPlayer(self, playerId: int):
    ...
    @staticmethod
    def UserToName(member: discord.Member, id: int):
        return member.name if member else str(id)

    def IdsToNames(self, ids: list[int]):
        return ", ".join(self.IdToName(id) for id in ids)

    def RemoveInGameFromQueue(self, game: Game):
        for id in game.AllPlayers:
            self.queues.remove_all_of(Region.ALL, id)

    async def StartGame(self, ctx, game: Game):
...
'''
    ...
    async def StartTeamPick(self, ctx, region: Region):
    ...
    async def ShowTeamPickInfo(self, ctx, game: Game, pingCapts = False):
    ...
    async def ReportMatchResult(self, ctx, res: str, playerId: int):           
    ...
    async def EndMatch(self, ctx, game: Game):
        if game == None:
            return
    ...
    async def PickPlayer(self, ctx, game: Game, choosingPlayer: str, pickedId: int, pickedPlayer: str):
    ...
    async def CreateStubs(self, ctx, region: Region, nStubs: int):
    ...
    async def CreateVoice(self, game: Game):
...
#
    ...
    async def SetVoicePerm(self, ids: list[int], channel: discord.VoiceChannel):
    ...
    async def DeleteVoice(self, game: Game):
    ...
    async def ValidateReg(self, ctx, region: Region):
        if not Region.Valid(region):
            await ctx.send(CodeSB(f'Invalid region, expected: {"/".join(Region.VALID_REGIONS)}'))
            return False
        return True

    async def ShowAddQueueStatus(self, ctx, playerName: str, regionsAddedTo: list[Region]):
    ...
    async def AddPlayerQueue(self, ctx, playerName: str, playerId: int, region: Region, showStatus=True):
    ...
    async def RemPlayerQueue(self, ctx, playerName: str, playerId: int, region: Region):
    ...
    async def AddPlayerSub(self, ctx, playerName: str, playerId: int, region: Region):
        regions = region.ToList()
    ...
    async def RemPlayerSub(self, ctx, playerName: str, playerId: int, region: Region):
        regions = region.ToList()
