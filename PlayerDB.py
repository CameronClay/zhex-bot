...
class PlayerDB:
    ...
    def Register(self, player):
        self.conn.execute('''insert into players (id, wins, loses, elo)
                    values (?, ?, ?, ?)''', (player.id, player.wins, player.loses, player.elo))
        self.conn.commit()
    ...
    def UpdateStats(self, player):
        self.conn.execute(f'''update players (id, wins, loses, elo) 
